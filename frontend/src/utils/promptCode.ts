...
/**
 * Preferred path: split on Markdown fences ``` / ```lang
 */
export function parseFencedSegments(prompt: string): PromptSegment[] | null {
  ...
  for (const line of lines) {
    ...
    // Inside a fence: bare ``` closes; ```lang starts a new block after flush.
    if (fence) {
      flushCode()
      if (fence[1]) {
        codeBuf = []
  ...
  // Unclosed fence: keep body as code (never re-inject ```lang into prose).
  flushCode()
  flushText()
...
/**
 * Legacy fallback: one prose block + optional trailing code (heuristic).
 */
...
/** Parse prompt into ordered text/code segments (fences preferred). */
